...
println("Type of l4 : $(typeof(l4))")
for i in l4
    println("l4 : $i")
end # note that 'end' is necessary here, see 'code blocks' below
...
# And as an equivalent of np.append
push!(l5, 36)
println("l5 : $l5")
...
for x in eachindex(l1)
    ...
    if l1[x] > 5
        leaky_if_block = "greater than 5" # For example replace that line by 'continue' (that will just skip the loop iteration) and an error will be raised because not all path assign a value to the varable 'leaky_if_block'
    elseif l1[x] < a 
        leaky_if_block = "smaller than $a" # here a has the value of the global variable defined l.11 (outer scope)
    else
        leaky_if_block = "It is between 3 and 5"
    end # 'end' to close if/elseif/else block
    println("Leaky if block : $leaky_if_block")
